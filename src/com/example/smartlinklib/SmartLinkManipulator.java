...
public class SmartLinkManipulator {
	...
	private Set<String> successEventMacSet = new HashSet<String>();
	//private Hashtable<String, ModuleInfo> successModule = new Hashtable<String, ModuleInfo>();
	...
	class ConfigUdpBroadcast {
		...
		public void sendHFA11Cmd() {
			...
			packetToSendbroadcast = new DatagramPacket(
					...
					"HF-A11ASSISTHREAD".getBytes().length, inetAddressbroadcast,
					48899);
			try {
				socket.send(packetToSendbroadcast);
			} catch (IOException e) {
...
//								System.out.println(receiveStr);
								...
								if (receiveStr.contains(RET_KEY)) {
									...
									if(!successMacSet.contains(mi.getMac())){
										...
										//successModule.put(mi.getMac(), mi);
									...
									if(info.length==3){
										...
										Log.e("RECV", receiveStr);
										ModuleInfo mi = new ModuleInfo();
										mi.setMac(info[1]);
										mi.setModuleIP(info[0]);
										mi.setMid(info[2]);
										if(successMacSet.contains(mi.getMac())){
											if(!successEventMacSet.contains(mi.getMac()))
											{
												callback.onConnect(mi);
												successEventMacSet.add(mi.getMac());
											}
										}
										
//										if(successModule.get(mi.getMac())!=null){
//											Log.e("RECV", "go throuth table");
//											callback.onConnect(mi);
//										}
									}
								}
